...
Stop 和直接调用 `auto-audit.py` 共用根定位，`--resolve-root` 只输出选中的目录，不运行审计或写缓存：

1. 直接调用的 `--root` 优先，其次是环境变量 `DOCS_GOVERNANCE_ROOT`。显式根须为存在的目录，且具有配置或旧四件套中的至少一份文件；无效即报告，不回退其他项目。相对路径按调用目录解析。
2. 无显式根时，从当前目录向上寻找最近的 `.docs-governance.json`，到当前 Git 工作区根即停止。独立子项目通过自己的配置声明边界，命中后不合并父配置；子配置禁用 Stop 或损坏时也不回退父配置。嵌套 Git 仓库不继承仓外规则；linked worktree 使用该工作区根。
3. Git 范围内没有配置时，仅在 Git 根识别旧四件套；模块级 `CLAUDE.md` 不自动成为独立项目。非 Git 目录沿祖先找最近配置，无配置再找最近的旧四件套位置。需要限定非 Git 范围时显式指定根。
4. 无法定位时输出“本次未执行审计”及配置提示，Stop 仍返回 0；直接 CLI 返回 2。成功定位后才进入该目录执行检查，缓存只保存在选中根的 `.governance/`。一次调用只选一个根，不自动遍历所有子项目；需审计独立子项目时分别调用。

同一项目从根、`src/` 或更深目录触发，使用同一配置和缓存。此规则已由直接 Bash 调用验证，不代表宿主的原生 Stop 事件派发已经验证。
